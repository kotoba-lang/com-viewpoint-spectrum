(ns viewpoint-spectrum.main
  "Kotodama WASM entrypoint for the Viewpoint_spectrum clean-room actor (L4) — Clojure port.
  ...
  facts (`viewpoint_spectrum.<Entity>/<field>`); `*store*` is the in-memory
  materialization used by the contract test and by the WASM runtime before
  a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "viewpoint_spectrum")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"viewpoint_spectrum.<Entity>/<field>\" v ...}.
  The datomic binding transacts these; the in-memory store keeps the record by id."
  [entity rec]
...
(defn healthz [] [{:status "ok" :actor "viewpoint_spectrum-compat" :tier tier :entities entities} 200])
...
(defn start! [] :viewpoint_spectrum-compat/ready)
