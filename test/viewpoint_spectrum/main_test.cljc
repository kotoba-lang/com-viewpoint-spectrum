(ns viewpoint-spectrum.main-test
  "Contract + behavioral test for the viewpoint_spectrum-compat L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "viewpoint_spectrum-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'viewpoint-spectrum.main-test)]
